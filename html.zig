...
const Element = union(enum) {
    ...
    pub fn init(allocator: Allocator, input: anytype) !Element {
        ...
        if (info == .Struct) {
            ...
            // TODO - handle list of elements case here
...
const Tag = struct {
    ...
    pub fn init(allocator: mem.Allocator, input: anytype) !Tag {
        ...
        if (input.len == 2) {
            if (unstd.isZigString(@TypeOf(input[1]))) {
                if (!matchEndTag(name, input[1])) return error.HtmlParseError;
                return .{ .name = name };
            } 
            const attributes = try parseAttributes(allocator, input[1]);
    ...
    pub fn parseAttributes(allocator: Allocator, input: anytype) ![]const Attribute {
        const input_type: type = @TypeOf(input);
        const info = @typeInfo(input_type);
        if (info != .Struct or (info.Struct.is_tuple and info.Struct.fields.len > 0)) {
            @compileError("Expected non-tuple struct. Received: " ++ @typeName(input_type));
        }
        const fields = info.Struct.fields;
        const attributes = try allocator.alloc(Attribute, fields.len);
        inline for (fields, 0..) |field, idx| {
            if (comptime !unstd.isZigString(field.type)) {
                @compileError("Expect string attribute. Received: " ++ @typeName(field.type));
            }
            attributes[idx] = .{ .name = field.name, .value = @field(input, field.name) };
        }
        return attributes;
    }
